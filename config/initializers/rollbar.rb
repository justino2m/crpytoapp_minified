...
if Rails.env.test? || Rails.env.development?
  Rollbar::Notifier.class_eval do
    def log(level, *args)
      unless args[0].to_s.start_with?('Matched ') # Matched bitrue-txns to wallet Test Wallet
        puts "rollbar #{level}: " + args[0].to_s + " #{args[1..-1].to_s}"
      end
      if args[0].is_a?(Exception)
        ap [args[0].message] + args[0].backtrace.take(10)
      end
    end
  end
end